...
- 다음 작업 때는 맥북에서도 같은 저장소를 열어 연습할 수 있도록, VS Code 작업 구성을 운영체제별로 나누는 방법을 정리해 볼 예정이다. 목표는 Windows에서는 현재처럼 MSVC를 쓰고, macOS에서는 `clang` 또는 `gcc`를 사용하는 별도 빌드 작업을 만들어 한 저장소를 두 환경에서 모두 무리 없이 쓰는 것이다.
